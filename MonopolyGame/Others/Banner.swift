...
func showSuccessBanner(title:String = "" , message: String) {
    let banner = Banner(title: title, subtitle: message + "\n", image: nil, backgroundColor: UIColor.init(named: "custom_theme_purple_colour")!)
    banner.show(duration: 4.0)
}

func showErrorBanner(message: String) {
    let banner = Banner(title: "", subtitle: message + "\n", image: nil, backgroundColor: UIColor.red)
...
//    }
    ...
    @objc class func topWindow() -> UIWindow? {
        for window in UIApplication.shared.windows.reversed() {
            if window.windowLevel + 1 == UIWindow.Level.normal + 1 && window.isKeyWindow && window.frame != CGRect.zero { return window }
        }
        return nil
    }
...
func showBannerMessage(flag:Bool, name:String) {
    if flag {
        showSuccessBanner(message: "Welcome to the \(name) Circle! We'll keep you notified on all updates.")
    } else {
        showErrorBanner(message: "You have left this Circle and will not be notified on updates.")
    }
}
